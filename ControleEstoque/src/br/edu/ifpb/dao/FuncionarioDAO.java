...
public class FuncionarioDAO {
	 ...
	 * usuario.setSexo(rs.getString("u.tp_sexo")); TipoUsuario tipoUsuario = new
	 * TipoUsuario(); tipoUsuario.setId(rs.getInt("u.cd_tipousuario"));
	 ...
	 * 
	 * try { // Define um update com os atributos e cada valor é representado //
	 * por // ? String sql = "UPDATE tb_usuario SET nm_senha=?" +
	 * " WHERE nm_login=?";
	 * 
	 * // prepared statement para inserção PreparedStatement stmt =
	 * (PreparedStatement) connection .prepareStatement(sql);
	 * 
	 * // seta os valores stmt.setString(1, user.getSenha()); stmt.setString(2,
	 * user.getLogin());
	 * 
	 * // envia para o Banco e fecha o objeto stmt.execute(); stmt.close();
	 * 
	 * } catch (SQLException sqle) { throw new RuntimeException(sqle); }
	 * 
	 * }
	 ...
	 * 
	 * // seta os valores stmt.setInt(1, usuario.getId());
	 * 
	 * // envia para o Banco e fecha o objeto stmt.execute(); stmt.close();
	 * 
	 ...
	 * while (rs.next()) {
	 * 
	 * Usuario user = new Usuario(); user.setId(rs.getInt("cd_usuario"));
	 ...
	 * user.setSexo(rs.getString("tp_sexo")); TipoUsuario tipoUsuario = new
	 * TipoUsuario(); tipoUsuario.setId(rs.getInt("cd_tipousuario"));
	 ...
	 * return users; }
	 * 
	 * public List<Usuario> consultarUsuariosByNome(Usuario usuario) throws
	 * SQLException {
	 * 
	 ...
	 * " T.nm_tipousuario" + " FROM tb_usuario as U, tb_tipousuario as T" +
	 * " WHERE U.nm_usuario LIKE ", usuario.getNome().trim() + "%",
	 ...
	 * ResultSet rs = stmt.executeQuery(sql);
	 * 
	 * while (rs.next()) { Usuario usuarioConsulta = new Usuario();
